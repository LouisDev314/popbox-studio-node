...
const sortRows = (sort: ProductSort) => {
  if (sort === 'price_asc') {
    return [asc(products.priceCents), desc(products.id)] as const;
  }

  if (sort === 'price_desc') {
    return [desc(products.priceCents), desc(products.id)] as const;
  }
...
const buildCursorCondition = (sort: ProductSort, cursor: ProductCursor | null): SQL | undefined => {
  ...
  if (sort === 'price_asc' && cursor.priceCents !== undefined && cursor.priceCents !== null) {
    return or(
  ...
  if (sort === 'price_desc' && cursor.priceCents !== undefined && cursor.priceCents !== null) {
    return or(
...
export const getProductBySlug = async (slug: string) => {
  const [row] = await db.select().from(products).where(and(eq(products.slug, slug), eq(products.status, 'active'))).limit(1);
...
export const getHomepageData = async () => {
  ...
  const allIds = Array.from(
    new Set([...newDrops, ...allProductsPreview, ...trendingFallback].map((row) => row.id)),
  );
