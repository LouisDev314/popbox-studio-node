...
const syncKujiInventory = async (productId: string) => {
  ...
  const [existingInventory] = await db.select().from(productInventory).where(eq(productInventory.productId, productId)).limit(1);
...
const replaceProductTags = async (productId: string, tagIds: string[]) => {
...
export const uploadProductImage = async (productId: string, file: Express.Multer.File, altText?: string | null) => {
  ...
  const { error } = await supabaseAdmin.storage.from(env.supabaseStorageBucket).upload(storageKey, file.buffer, {
